...
from anvil.cost.inference_cost import build_self_hosted, compare
...
def test_task_metric_comparison_returns_one_bar_group_per_field() -> None:
    ...
    ax = fig.axes[0]
    # 3 variants by 2 fields = 6 bars total.
    bars = ax.containers
    assert len(bars) == 3
...
def test_cost_per_1m_tokens_renders_one_bar_per_scenario() -> None:
    ...
    ax = fig.axes[0]
    bars = ax.containers[0]
    assert len(bars) == 3  # 1 self-hosted + 2 API
...
def test_breakeven_curve_draws_crossover_marker_when_curves_cross() -> None:
    # API >> fine-tuned, training low; crossover happens early.
    points = cumulative_cost_curve(
